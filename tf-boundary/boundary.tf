...
provider "boundary" {
  addr                            = "http://boundary-server:9200"
  auth_method_id                  = "ampw_L3MWNBCGH0"
  password_auth_method_login_name = "admin"
  password_auth_method_password   = "5pgCIeD7pMdG0LHKshNA"
}
...
# added to test connect ssh target (start) ----
variable "ips_sshtgt" {
  type    = set(string)
  default = [
    "sshd_target1",
...
resource "boundary_host_catalog" "hostcat_sshtgt" {
  name        = "hostcat_sshtgt"
  description = "Backend servers host catalog"
  type        = "static"
  scope_id    = boundary_scope.core_infra.id
...
resource "boundary_host" "host_sshtgt" {
  for_each        = var.ips_sshtgt
  type            = "static"
  name            = "sshtgt_service_${each.value}"
  description     = "Backend server host"
  address         = each.key
  host_catalog_id = boundary_host_catalog.hostcat_sshtgt.id
}

resource "boundary_host_set" "hostset_sshtgt" {
  type            = "static"
  name            = "hostset_sshtgt"
  description     = "Host set for backend servers"
  host_catalog_id = boundary_host_catalog.hostcat_sshtgt.id
  host_ids        = [for host in boundary_host.host_sshtgt : host.id]
}

# create target for accessing backend servers on port :22
resource "boundary_target" "target_sshtgt" {
  type         = "tcp"
  name         = "sshd_target1"
  ...
  host_set_ids = [
    boundary_host_set.hostset_sshtgt.id
  ]
}
# added to test connect ssh target (end) ----
